...
var (
	...
	timeout = flag.Uint(
		"t", 5000, "Timeout for all connections in milliseconds")
...
func main() {
	...
	if *verbose {
		log.Println(
			"Initializing download with", *nConns, "concurrent connections")
	}

	// Initialize download
	dldr := md.NewMultiDownloader(
		flag.Args(),
		int(*nConns),
	...
	if *verbose {
		...
		err = dldr.Download(
			func(feedback []md.ConnectionProgress) {
				v.Update(feedback)
			})
	} else {
